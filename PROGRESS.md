...
- Business logos use the local upload directory in SQLite mode and private tenant-scoped Supabase Storage in hosted mode. Production malware scanning is implemented and fail-closed; live scanner deployment/configuration and optional image transformation remain.
...
- Probabilistic cash analytics, external API keys/customer webhooks, optional image transformation, and a completed live disaster-recovery drill. Distributed rate limits and fail-closed attachment malware scanning are implemented but still need their production Redis/scanner services; vendor-neutral external error and metric export still needs a production monitoring destination and scraper.
