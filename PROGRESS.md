...
- Full hosted production SaaS brief: approximately 98% complete.
...
- Branding release pass: the Forma logo is a tracked browser asset; PNG, JPEG, WebP, and hardened SVG business-logo uploads are rasterized into bounded metadata-free PNGs before Storage so the same managed mark renders in HTML, generated PDFs, and email attachments. Legacy managed WebP/SVG assets are normalized on PDF read, unsafe SVG external-resource constructs are rejected, and the packaged Forma logo remains the fallback. Automated coverage proves browser asset rendering, PNG and vector-source PDF embedding, WebP/SVG normalization, corrupt-image rejection, and lossless migration of the legacy default business identity.
...
- Production runtime hardening: every response carries a request ID, strict content/security/referrer/permissions headers, API no-store policy, and bounded rate limits. An Upstash-compatible Redis REST adapter applies atomic fixed-window counters across application instances, HMAC-pseudonymizes IP identities, falls back to a capped 10,000-bucket in-process safety net during brief Redis outages, and prevents production readiness without distributed configuration. Production emits structured request/error/shutdown logs without request bodies or credentials, exports low-cardinality Prometheus metrics behind a separate timing-safe secret, can send HMAC-signed sanitized server-error envelopes to an HTTPS sink, actively checks hosted readiness, and performs graceful shutdown.
...
- Executable data cutover and recovery tooling: SQLite and hosted workspaces export into tenant-bound portable bundles with per-table/object SHA-256 checksums, private-object copies, exact schema checks, legacy-audit preservation, and path traversal protection. PostgreSQL connections verify remote TLS certificates, imports require an explicit existing empty workspace, inject that tenant on every row, handle document/recurring cycles, commit atomically only after exact row-count verification, resume interrupted object uploads separately, and verify hosted rows and objects against the manifest. Portable hosted exports support encrypted off-platform workspace inventories while managed backup/PITR remains the whole-project recovery authority.
- Production release gate: one non-secret-emitting command checks fail-closed runtime settings, HTTPS/public and scheduler/metrics boundaries, distributed rate limiting, malware scanning, Supabase server credentials, declared Auth providers, Resend, Stripe, PayPal, and monitoring configuration. Remote checks verify the Redis store, a clean scanner probe, Auth provider enablement, service-role access, the private bucket, the full migration ledger, deployed `/api/ready`, a verified Resend sender domain, and accepted live Stripe/PayPal credentials; blockers produce a non-zero exit.
...
- Supporting-document attachments upload from both invoice entry flows. PDFs and supported images are signature-checked, scanned before persistence in production, stored locally or in private tenant-scoped Supabase Storage, listed on drafts, served through authenticated asset requests, and removable before issue. The live scanner endpoint remains external deployment configuration.
...
- Probabilistic cash analytics, external API keys/customer webhooks, and a completed live disaster-recovery drill. Distributed rate limits, fail-closed attachment malware scanning, and logo normalization are implemented but still need their production Redis/scanner services; vendor-neutral external error and metric export still needs a production monitoring destination and scraper.
...
3. Deploy behind the production HTTPS origin, provision the Redis rate-limit store and malware scanner, connect metrics/error exports to the monitoring stack, enable managed backups/PITR, run `npm run production:check`, and complete the first documented isolated restore drill.
