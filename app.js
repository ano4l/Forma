...
const invitationHash = (() => { if (!location.hash.startsWith("#invite=")) return ""; try { return decodeURIComponent(location.hash.slice(8)); } catch { return ""; } })();
const state = { route: invitationHash ? "documents" : location.hash.slice(1) || "documents", invoices: [], customers: [], products: [], current: null, audit: [], customerQuery: "", productQuery: "", invoiceQuery: "", invoiceStatus: "all", saveTimer: null, savePromise: null, editRevision: 0, savedRevision: 0, saving: false, saveError: false, touched: new Set(), expandedLines: new Set(), documents: [], document: null, templates: [], profile: {}, paymentMethods: [], emailTemplates: [], brandingPresets: [], numberPrefixes: {}, retentionPolicy: { payload_days: 90, legal_hold: false }, settingsEmailPurpose: "", documentQuery: "", documentType: "all", documentCustomerQuery: "", documentProductQuery: "", quickText: "", quickParsed: null, documentSaving: false, documentSaveError: false, documentSaveTimer: null, documentSavePromise: null, documentEditRevision: 0, documentSavedRevision: 0, documentEmailHistory: [], documentPaymentLinks: [], documentRefunds: [], documentDisputes: [], documentUndo: null, recurringSchedules: [], reminderRules: [], dueReminders: [], workspaceTeam: { members: [], invitations: [] } };
...
let pendingInvitationToken = invitationHash;
if (pendingInvitationToken) history.replaceState(null, "", `${location.pathname}${location.search}`);
const authorizedAssetUrls = new Map();
const canWriteWorkspace = () => !hostedSession || hostedSession.role !== "viewer";
const canAdminWorkspace = () => !hostedSession || ["owner", "admin"].includes(hostedSession.role);
const routes = [
...
function renderReports() {
  ...
  return `<div class="page reports-page">${pageHead("Insights", "Receivables report", "A ledger-level view of billed, collected, outstanding, aging, tax, and expected collection timing.", `<button class="btn primary" data-export-aging-pdf>${icon("download")}Formal PDF</button>`)}<div class="dashboard-grid report-metrics">${[["Billed", money(billed), "Issued invoices"], ["Collected", money(collected), "Recorded payments"], ["Outstanding", money(outstanding), "Open invoice balances"], ["Tax recorded", money(tax), "Across issued invoices"]].map(([label, value, note]) => `<section class="metric-card"><span>${label}</span><strong>${value}</strong><small>${note}</small></section>`).join("")}</div><section class="panel report-export-note"><div><h2>Finance exports</h2><p class="panel-sub">Download spreadsheet-safe ledgers, aging and collection schedules, tax-by-rate detail, or a presentation-ready aging PDF.</p></div><div class="modal-actions"><button class="btn" data-export-receivables>${icon("download")}Ledger CSV</button><button class="btn" data-export-aging>${icon("download")}Aging CSV</button><button class="btn" data-export-forecast>${icon("download")}Collection forecast</button><button class="btn" data-export-tax>${icon("download")}Tax CSV</button><button class="btn primary" data-export-aging-pdf>${icon("download")}Aging PDF</button></div></section><section class="table-card invoice-table report-table"><div class="data-row head"><span>Document</span><span>Customer</span><span>Total</span><span>Balance</span><span>Status</span><span></span></div>${entries.length ? entries.map((document) => `<div class="data-row"><span><strong>${escapeHtml(document.number)}</strong><small>${documentTypeLabel(document.document_type)} &middot; ${dateLabel(document.data?.issue_date)}</small></span><span><strong>${escapeHtml(documentCustomer(document))}</strong><small>${escapeHtml(document.data?.currency || "ZAR")}</small></span><strong>${money(documentTotal(document), document.data?.currency)}</strong><span><strong>${money(document.balance_due_minor ?? documentTotal(document), document.data?.currency)}</strong><small>Balance due</small></span><span><span class="status ${escapeHtml(document.status)}">${escapeHtml(document.status)}</span></span><button class="btn" data-open-document="${escapeHtml(document.id)}">Open</button></div>`).join("") : `<div class="table-empty"><strong>No receivables data yet</strong><span>Create and issue invoices to populate this report.</span></div>`}</section></div>`;
}
...
function documentActionMarkup(document) {
  ...
  if (type === "invoice" && document.amount_paid_minor > 0 && ["paid", "partially_paid", "overdue"].includes(status) && canAdminWorkspace()) actions.push(action("refund", "Issue refund"));
  if (status !== "draft") actions.push(action("portal", `${icon("copy")}Share portal`));
...
function financialHistoryMarkup() {
  if (state.document?.document_type !== "invoice") return "";
  const rows = [...state.documentRefunds.map((refund) => ({ kind: "Refund", status: refund.status, amount: refund.amount_minor, currency: refund.currency, when: refund.updated_at, detail: refund.provider })), ...state.documentDisputes.map((dispute) => ({ kind: "Dispute", status: dispute.status, amount: dispute.amount_minor, currency: dispute.currency, when: dispute.updated_at, detail: dispute.reason || dispute.provider }))].sort((a, b) => String(b.when).localeCompare(String(a.when)));
  return `<section class="panel"><h2>Payment adjustments</h2><p class="panel-sub">Provider-confirmed refunds update the ledger once. Disputes are tracked without silently changing receivables.</p>${rows.length ? `<div class="email-history">${rows.slice(0, 6).map((row) => `<div class="email-history-row ${["succeeded", "completed", "won", "resolved"].includes(String(row.status).toLowerCase()) ? "accepted" : "failed"}"><span><strong>${escapeHtml(row.kind)} · ${escapeHtml(String(row.status).replaceAll("_", " "))}</strong><small>${row.amount ? money(row.amount, row.currency || state.document.data.currency) : "Amount unavailable"} · ${escapeHtml(row.detail || "provider event")}</small></span></div>`).join("")}</div>` : `<p class="panel-sub">No refunds or disputes recorded.</p>`}</section>`;
}
...
function renderSettings() {
  ...
  const team = state.workspaceTeam || { members: [], invitations: [] }; const activeInvitations = team.invitations.filter((invitation) => !invitation.accepted_at && new Date(invitation.expires_at) > new Date());
  const teamPanel = hostedSession && ["owner", "admin"].includes(hostedSession.role) ? `<section class="panel settings-panel"><div class="settings-panel-head"><div><h2>Workspace team</h2><p class="panel-sub">Invite people and control workspace access. Invitation secrets are shown once.</p></div><span class="settings-kicker">Access</span></div><div class="payment-method-list">${team.members.map((member) => `<div class="payment-method"><span><strong>${member.user_id === hostedSession.user?.id ? escapeHtml(hostedSession.user.email || "You") : `User ${escapeHtml(member.user_id.slice(0,8))}`}</strong><small>${escapeHtml(member.status)} workspace access</small></span><div class="modal-actions"><select aria-label="Role" data-member-role="${escapeHtml(member.user_id)}">${["owner","admin","member","viewer"].map((role) => `<option value="${role}" ${member.role === role ? "selected" : ""}>${role}</option>`).join("")}</select><select aria-label="Status" data-member-status="${escapeHtml(member.user_id)}"><option value="active" ${member.status === "active" ? "selected" : ""}>active</option><option value="suspended" ${member.status === "suspended" ? "selected" : ""}>suspended</option></select><button class="link-btn" data-save-member="${escapeHtml(member.user_id)}">Save</button></div></div>`).join("") || `<p class="panel-sub">No members returned.</p>`}</div><h3>Pending invitations</h3><div class="payment-method-list">${activeInvitations.map((invitation) => `<div class="payment-method"><span><strong>${escapeHtml(invitation.email)}</strong><small>${escapeHtml(invitation.role)} · expires ${dateLabel(String(invitation.expires_at).slice(0,10))} · ${escapeHtml(invitation.delivery_status || "pending")}</small></span><button class="link-btn" data-revoke-invitation="${escapeHtml(invitation.id)}">Revoke</button></div>`).join("") || `<p class="panel-sub">No pending invitations.</p>`}</div><div class="field-grid">${simpleInput("Email address", "invitationEmail", "", "email", { placeholder: "colleague@example.com" })}<div class="field"><label for="invitationRole">Role</label><div class="input-wrap"><select id="invitationRole"><option value="member">Member</option><option value="admin">Admin</option><option value="viewer">Viewer</option></select></div></div></div><div class="modal-actions"><button class="btn primary" data-send-invitation>${icon("send")}Send invitation</button></div></section>` : "";
  const retention = state.retentionPolicy || { payload_days: 90, legal_hold: false }; const retentionPanel = `<section class="panel settings-panel"><div class="settings-panel-head"><div><h2>Provider payload retention</h2><p class="panel-sub">Event IDs remain for idempotency; raw callback bodies are redacted after this window to reduce retained personal data.</p></div><span class="settings-kicker">Privacy</span></div><div class="field-grid">${simpleInput("Raw payload days", "retentionPayloadDays", retention.payload_days || 90, "number", { min: 7, step: 1 })}<label class="checkbox-field"><input id="retentionLegalHold" type="checkbox" ${retention.legal_hold ? "checked" : ""}>Legal hold pauses redaction</label></div><div class="modal-actions">${canAdminWorkspace() ? `<button class="btn" data-save-retention>${icon("check")}Save retention policy</button>` : `<span class="panel-sub">Workspace admin access is required to change this policy.</span>`}</div></section>`;
  return `<div class="page settings-page">${pageHead("System", "Business settings", "Set the identity and payment details used by new documents.")}
    ...
    <section class="panel settings-email settings-panel"><div class="settings-panel-head"><div><h2>Email templates</h2><p class="panel-sub">Templates produce the default subject and message for configured delivery.</p></div><span class="settings-kicker">Delivery</span></div>${state.emailTemplates.length ? `<div class="field"><label for="emailTemplatePurpose">Message type</label><div class="input-wrap"><select id="emailTemplatePurpose">${state.emailTemplates.map((template) => `<option value="${escapeHtml(template.purpose)}" ${template.purpose === selectedPurpose ? "selected" : ""}>${escapeHtml(template.purpose.replaceAll("_", " "))}</option>`).join("")}</select></div></div>${simpleInput("Subject", "emailTemplateSubject", selectedTemplate.subject || "", "text")}<div class="field"><label for="emailTemplateText">Plain text body</label><div class="input-wrap textarea-wrap"><textarea id="emailTemplateText" rows="6">${escapeHtml(selectedTemplate.text || selectedTemplate.body || "")}</textarea></div></div><div class="modal-actions"><button class="btn" data-save-email-template>${icon("check")}Save email template</button></div>` : `<p class="panel-sub">No templates returned yet.</p>`}</section>${retentionPanel}${teamPanel}</div></div>`;
}
...
async function loadWorkspaceTeam() { if (!hostedSession || !["owner", "admin"].includes(hostedSession.role)) { state.workspaceTeam = { members: [], invitations: [] }; return; } state.workspaceTeam = await api("/api/workspace/team"); }
...
async function loadDocumentFinancials(id = state.document?.id) { if (!id || state.document?.document_type !== "invoice") { state.documentPaymentLinks = []; state.documentRefunds = []; state.documentDisputes = []; return; } const [links, refunds, disputes] = await Promise.all([api(`/api/documents/${id}/payment-links`), api(`/api/documents/${id}/refunds`), api(`/api/documents/${id}/disputes`)]); state.documentPaymentLinks = links || []; state.documentRefunds = refunds || []; state.documentDisputes = disputes || []; }
async function loadDocumentSupport() {
  const [templates, profile, paymentMethods, emailTemplates, brandingPresets, numberPrefixes, retentionPolicy] = await Promise.all([api("/api/templates"), api("/api/business-profile"), api("/api/payment-methods"), api("/api/email-templates"), api("/api/branding-presets"), api("/api/number-prefixes"), api("/api/privacy/provider-payload-retention")]);
  state.templates = templates || []; state.profile = profile || {}; state.paymentMethods = paymentMethods || []; state.emailTemplates = emailTemplates || []; state.brandingPresets = brandingPresets || []; state.numberPrefixes = numberPrefixes || {}; state.retentionPolicy = retentionPolicy || { payload_days: 90, legal_hold: false };
  await hydrateAuthorizedAsset(state.profile.logo_url);
}
async function openDocument(id) {
  try { if (state.document?.status === "draft") await saveDocument({ quiet: true }); state.document = await api(`/api/documents/${id}`); state.documentEditRevision = 0; state.documentSavedRevision = 0; state.documentSaveError = false; await Promise.all([loadDocumentSupport(), loadDocumentEmailHistory(id), loadDocumentFinancials(id)]); setRoute("document-editor"); }
...
async function openRefundDialog() {
  try {
    await loadDocumentFinancials(state.document.id);
    const reserved = (checkoutId) => state.documentRefunds.filter((refund) => refund.checkout_id === checkoutId && !["failed", "canceled"].includes(refund.status)).reduce((sum, refund) => sum + Number(refund.amount_minor || 0), 0);
    const checkouts = state.documentPaymentLinks.map((checkout) => ({ ...checkout, refundable: Math.max(0, Number(checkout.amount_minor || 0) - reserved(checkout.id)) })).filter((checkout) => checkout.status === "paid" && checkout.provider_payment_id && checkout.refundable > 0);
    if (!checkouts.length) return toast("No provider payment has a refundable balance.", "error");
    const options = checkouts.map((checkout) => `<option value="${escapeHtml(checkout.id)}" data-refundable="${checkout.refundable}">${escapeHtml(checkout.provider.toUpperCase())} · ${money(checkout.refundable, checkout.currency)} available</option>`).join("");
    const history = state.documentRefunds.length ? `<div class="email-history">${state.documentRefunds.slice(0, 5).map((refund) => `<div class="email-history-row ${refund.status === "succeeded" ? "accepted" : "failed"}"><span><strong>${money(refund.amount_minor, refund.currency)} · ${escapeHtml(refund.status)}</strong><small>${escapeHtml(refund.provider.toUpperCase())} · ${escapeHtml(refund.reason || "No reason supplied")}</small></span></div>`).join("")}</div>` : "";
    $("#modalContent").innerHTML = `<h2>Issue provider refund</h2><p class="lead">Refunds return money through the original provider. The invoice ledger changes only after provider confirmation.</p><div class="field-grid"><div class="field span-2"><label for="refundCheckout">Payment</label><div class="input-wrap"><select id="refundCheckout">${options}</select></div></div>${simpleInput("Amount", "refundAmount", (checkouts[0].refundable / 100).toFixed(2), "number", { min: 0.01, step: 0.01 })}<div class="field"><label for="refundReason">Reason</label><div class="input-wrap"><select id="refundReason"><option value="requested_by_customer">Customer request</option><option value="duplicate">Duplicate payment</option><option value="fraudulent">Fraudulent payment</option><option value="">Other / unspecified</option></select></div></div></div>${history}<div class="email-compose-note">This action cannot be undone in Forma. Provider processing may remain pending before it succeeds.</div><div class="modal-actions"><button class="btn" value="cancel">Cancel</button><button class="btn danger" type="button" id="confirmRefund">Issue refund</button></div>`;
    $("#modal").showModal(); $("#refundCheckout").onchange = () => { const option = $("#refundCheckout").selectedOptions[0]; $("#refundAmount").value = (Number(option.dataset.refundable) / 100).toFixed(2); }; $("#confirmRefund").onclick = issueRefund;
  } catch (error) { toast(`Refunds could not be loaded: ${error.message}`, "error"); }
}
async function issueRefund() {
  const button = $("#confirmRefund"); button.disabled = true; button.textContent = "Submitting...";
  try {
    const result = await api(`/api/documents/${state.document.id}/refunds`, { method: "POST", headers: { "Idempotency-Key": crypto.randomUUID() }, body: JSON.stringify({ checkout_id: $("#refundCheckout").value, amount_minor: Math.round((Number($("#refundAmount").value) || 0) * 100), reason: $("#refundReason").value }) });
    state.document = result.invoice || await api(`/api/documents/${state.document.id}`); await Promise.all([loadDocumentFinancials(state.document.id), refreshDocuments(), refreshReminders()]); $("#modal").close(); render(); toast(result.refund?.status === "succeeded" ? "Refund completed and ledger updated" : `Refund submitted: ${result.refund?.status || "pending"}`);
  } catch (error) { button.disabled = false; button.textContent = "Issue refund"; toast(`Refund not issued: ${error.message}`, "error"); }
}
async function openPortalLinkDialog() {
  try {
    const links = await api(`/api/documents/${state.document.id}/portal-links`); const active = links.filter((link) => !link.revoked_at && new Date(link.expires_at) > new Date());
    $("#modalContent").innerHTML = `<h2>Share secure customer portal</h2><p class="lead">Create an opaque, expiring link for document review, PDF download, payment, or quote response. The secret is shown once.</p><div class="field-grid">${simpleInput("Expires in days", "portalExpiryDays", "30", "number", { min: 1, step: 1 })}</div><div id="portalLinkResult">${active.length ? `<p class="panel-sub">${active.length} active link${active.length === 1 ? "" : "s"}. Existing secrets cannot be displayed again; revoke them if no longer needed.</p>${active.map((link) => `<div class="email-history-row accepted"><span><strong>Active until ${dateLabel(String(link.expires_at).slice(0,10))}</strong><small>${link.last_viewed_at ? `Last opened ${new Date(link.last_viewed_at).toLocaleString("en-ZA")}` : "Not opened yet"}</small></span><button class="link-btn" data-revoke-portal-link="${escapeHtml(link.id)}">Revoke</button></div>`).join("")}` : `<p class="panel-sub">No active customer portal links.</p>`}</div><div class="modal-actions"><button class="btn" value="cancel">Close</button><button class="btn primary" type="button" id="createPortalLink">Create secure link</button></div>`;
    $("#modal").showModal(); $("#createPortalLink").onclick = createPortalLink; $$('[data-revoke-portal-link]', $("#modalContent")).forEach((button) => button.onclick = () => revokePortalLink(button.dataset.revokePortalLink));
  } catch (error) { toast(`Portal links could not be loaded: ${error.message}`, "error"); }
}
async function createPortalLink() {
  const button = $("#createPortalLink"); button.disabled = true;
  try { const link = await api(`/api/documents/${state.document.id}/portal-links`, { method: "POST", body: JSON.stringify({ expires_in_days: Number($("#portalExpiryDays").value) || 30 }) }); $("#portalLinkResult").innerHTML = `<div class="email-compose-note"><strong>Secure portal link ready</strong><p>This secret is shown once. Copy it before closing.</p><div class="input-wrap"><input id="portalLinkUrl" value="${escapeHtml(link.url)}" readonly></div><div class="modal-actions"><button class="btn primary" type="button" id="copyPortalLink">Copy secure link</button><a class="btn" href="${escapeHtml(link.url)}" target="_blank" rel="noopener">Preview portal</a></div></div>`; $("#copyPortalLink").onclick = async () => { await navigator.clipboard.writeText(link.url); toast("Secure portal link copied"); }; toast("Customer portal link created"); }
  catch (error) { button.disabled = false; toast(`Portal link not created: ${error.message}`, "error"); }
}
async function revokePortalLink(id) { try { await api(`/api/portal-links/${id}`, { method: "DELETE" }); await openPortalLinkDialog(); toast("Portal link revoked"); } catch (error) { toast(`Portal link not revoked: ${error.message}`, "error"); } }
async function runDocumentAction(action) {
  ...
  if (action === "refund") return openRefundDialog();
  if (action === "portal") return openPortalLinkDialog();
...
async function saveProviderRetention() {
  try { state.retentionPolicy = await api("/api/privacy/provider-payload-retention", { method: "PUT", body: JSON.stringify({ payload_days: Number($("#retentionPayloadDays").value), legal_hold: Boolean($("#retentionLegalHold").checked) }) }); render(); toast("Provider payload retention updated"); }
  catch (error) { toast(`Retention policy not saved: ${error.message}`, "error"); }
}
...
async function sendWorkspaceInvitation() {
  const email = $("#invitationEmail")?.value.trim(); const role = $("#invitationRole")?.value || "member"; if (!email) return toast("Enter an email address", "error");
  try { const invitation = await api("/api/workspace/invitations", { method: "POST", body: JSON.stringify({ email, role }) }); await loadWorkspaceTeam(); render(); $("#modalContent").innerHTML = `<h2>Invitation created</h2><p class="lead">${invitation.delivery?.accepted ? "Delivery was accepted by the configured provider." : "Delivery was not accepted. Copy the secure link and send it through a trusted channel."}</p><div class="input-wrap"><input id="invitationUrl" value="${escapeHtml(invitation.invite_url)}" readonly></div><div class="modal-actions"><button class="btn" value="cancel">Close</button><button class="btn primary" type="button" id="copyInvitationUrl">Copy invitation link</button></div>`; $("#modal").showModal(); $("#copyInvitationUrl").onclick = async () => { await navigator.clipboard.writeText(invitation.invite_url); toast("Invitation link copied"); }; }
  catch (error) { toast(`Invitation not created: ${error.message}`, "error"); }
}
async function revokeWorkspaceInvitation(id) { try { await api(`/api/workspace/invitations/${id}`, { method: "DELETE" }); await loadWorkspaceTeam(); render(); toast("Invitation revoked"); } catch (error) { toast(`Invitation not revoked: ${error.message}`, "error"); } }
async function saveWorkspaceMember(userId) { try { const role = document.querySelector(`[data-member-role="${CSS.escape(userId)}"]`).value; const status = document.querySelector(`[data-member-status="${CSS.escape(userId)}"]`).value; await api(`/api/workspace/members/${encodeURIComponent(userId)}`, { method: "PATCH", body: JSON.stringify({ role, status }) }); await loadWorkspaceTeam(); render(); toast("Member access updated"); } catch (error) { toast(`Member access not updated: ${error.message}`, "error"); } }
...
function bindPage() {
  ...
  $$('[data-export-aging]').forEach((el) => el.onclick = () => openAuthorizedResource("/api/reports/aging.csv", { download: true }));
  $$('[data-export-tax]').forEach((el) => el.onclick = () => openAuthorizedResource("/api/reports/tax.csv", { download: true }));
  $$('[data-export-forecast]').forEach((el) => el.onclick = () => openAuthorizedResource("/api/reports/collection-forecast.csv", { download: true }));
  $$('[data-export-aging-pdf]').forEach((el) => el.onclick = () => openAuthorizedResource("/api/reports/receivables.pdf", { download: true }));
  ...
  $$('[data-save-retention]').forEach((el) => el.onclick = saveProviderRetention);
  $$('[data-save-email-template]').forEach((el) => el.onclick = saveEmailTemplate);
  $$('[data-send-invitation]').forEach((el) => el.onclick = sendWorkspaceInvitation);
  $$('[data-revoke-invitation]').forEach((el) => el.onclick = () => revokeWorkspaceInvitation(el.dataset.revokeInvitation));
  $$('[data-save-member]').forEach((el) => el.onclick = () => saveWorkspaceMember(el.dataset.saveMember));
...
async function bootstrap() {
  try {
    await Promise.all([refreshLists(), refreshDocuments(), refreshRecurringSchedules(), refreshReminders(), loadDocumentSupport(), loadWorkspaceTeam()]);
    ...
    if (initialRoute === "document-editor" && initialDocumentId) { state.route = "document-editor"; state.documentRouteId = initialDocumentId; state.document = await api(`/api/documents/${initialDocumentId}`); await Promise.all([loadDocumentEmailHistory(initialDocumentId), loadDocumentFinancials(initialDocumentId)]); }
    render();
...
async function loadHostedSession() {
  try {
    ...
    if (pendingInvitationToken) { try { const accepted = await api("/api/workspaces/accept-invitation", { method: "POST", body: JSON.stringify({ token: pendingInvitationToken }), skipWorkspace: true }); pendingInvitationToken = ""; localStorage.setItem(WORKSPACE_KEY, accepted.workspace_id); session = await api("/api/session", { skipWorkspace: true }); hostedSession = session; } catch (error) { pendingInvitationToken = ""; return session.memberships.length ? renderWorkspaceChooser(session, error.message) : renderWorkspaceOnboarding(session, error.message); } }
...
window.addEventListener("hashchange", async () => { const [next, documentId] = location.hash.slice(1).split("/"); if (!next) return; if (next === "invoice") return setRoute("documents"); if (next === "document-editor" && documentId) { try { state.route = next; state.documentRouteId = documentId; state.document = await api(`/api/documents/${documentId}`); await Promise.all([loadDocumentSupport(), loadDocumentEmailHistory(documentId), loadDocumentFinancials(documentId)]); render(); } catch (error) { toast(error.message, "error"); setRoute("documents"); } return; } if (next !== state.route) { state.route = next; state.documentRouteId = null; state.documentEmailHistory = []; state.documentPaymentLinks = []; state.documentRefunds = []; state.documentDisputes = []; render(); } });
